  ...
  Future<TokenResponse> _getTokenFromBackend(String code, String? state) async {
    try {
      ...
      if (widget.loginType == 'kakao') {
        ...
        tokenUrl = dotenv.env['KAKAO_CALLBACK_URL'] ?? 'http://10.0.2.2:8080/api/oauth2/callback/kakao/token';
      } else {
        // 네이버도 v1 없이 동일한 패턴으로 수정
        // tokenUrl = 'http://10.0.2.2:8080/api/oauth2/callback/naver/token';
        tokenUrl = dotenv.env['NAVER_CALLBACK_URL'] ?? 'http://10.0.2.2:8080/api/oauth2/callback/naver/token';
      }
