...
class ModernSocialLoginButton extends StatelessWidget {
  ...
  final String kakaoClientId = dotenv.env['KAKAO_CLIENT_ID'] ?? '357d3401893dc5c9cbefc83bb65df4ee';
  final String kakaoCallbackUrl = dotenv.env['KAKAO_CALLBACK_URL'] ?? 'http://localhost:8080/api/oauth2/callback/kakao';
  final String naverClientId = dotenv.env['NAVER_CLIENT_ID'] ?? '3fsFDFBv3hFuJkDoYMva';
  final String naverCallbackUrl = dotenv.env['NAVER_CALLBACK_URL'] ?? 'http://localhost:8080/api/v1/oauth2/callback/naver';
  ...
  void _handleLogin(BuildContext context) async {
    try {
      ...
      final redirectUrl = type == SocialLoginType.kakao
          ? kakaoCallbackUrl // 카카오는 v1 없음
          : naverCallbackUrl;  // 네이버는 v1 있음
    ...
    switch (type) {
      case SocialLoginType.kakao:
        ...
        return 'https://kauth.kakao.com/oauth/authorize'
            '?client_id=$kakaoClientId'  // 이제 변수 사용
            '&redirect_uri=$redirectUri'
            '&response_type=code';
      case SocialLoginType.naver:
        final state = DateTime.now().millisecondsSinceEpoch.toString();
        return 'https://nid.naver.com/oauth2.0/authorize'
            '?client_id=$naverClientId'  // 이제 변수 사용
            '&response_type=code'
            '&redirect_uri=${Uri.encodeComponent(naverCallbackUrl)}'
            '&state=$state';
    }
  }
